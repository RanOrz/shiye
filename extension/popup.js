...
function extractUrl(value) { const match = String(value || "").match(/https?:\/\/[^\s<>"】）)]+/i); return match ? match[0].replace(/[.,!?。，！？】）)]+$/, "") : String(value || "").trim(); }
function isVideoUrl(value) { try { const url = new URL(value); return /(^|\.)((youtube\.com)|(youtu\.be)|(vimeo\.com)|(bilibili\.com)|(b23\.tv)|(soundcloud\.com))$/i.test(url.hostname) || /\.(mp3|mp4|m4a|webm|mov|wav|ogg)(?:$|\?)/i.test(url.pathname); } catch { return false; } }
...
function setActiveMode(mode) { activeMode = mode; document.querySelectorAll(".mode-tab").forEach((item) => item.classList.toggle("is-active", item.dataset.mode === mode)); elements.resourceUrl.placeholder = mode === "media" ? "粘贴音视频链接" : (pageData ? "本页已读取，可直接保存" : "点击【本页】提取当前页面"); elements.pagePreview.classList.toggle("is-hidden", mode !== "page" || !pageData); elements.jobCard.classList.toggle("is-hidden", mode !== "media"); elements.resourceSubmit.querySelector("span").textContent = mode === "media" ? "开始转写" : (pageData ? "保存正文" : "保存到本地"); }
async function fillCurrentUrl() { try { const tab = await getCurrentTab(); if (isVideoUrl(tab.url)) { setActiveMode("media"); elements.resourceUrl.value = tab.url; } else { setActiveMode("page"); await extractCurrentPage(); } } catch (error) { showMessage(error.message, "error"); } }
...
async function submitResource(event) { event.preventDefault(); hideMessage(); const url = extractUrl(elements.resourceUrl.value); elements.resourceUrl.value = url; const mediaMode = activeMode === "media" || isVideoUrl(url); if (mediaMode && !url) return showMessage("请先粘贴音视频链接。", "error"); if (!mediaMode && !pageData) return showMessage("请先点击“本页”读取正文。", "error"); elements.resourceSubmit.disabled = true; try { if (mediaMode) await startMediaJob(); else await saveCurrentPage(); } finally { if (!mediaMode) elements.resourceSubmit.disabled = false; } }
...
document.querySelectorAll(".mode-tab").forEach((tab) => tab.addEventListener("click", () => setActiveMode(tab.dataset.mode)));
